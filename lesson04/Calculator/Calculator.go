...
func main() {
	input := ""
	for {
		fmt.Println("Введи выражение, что нужно посчитать. (help - справка, exit - выход) ")
		fmt.Print(">")
		...
		if input == "exit" {
			fmt.Println("Калькулятор закрыт!")
			break
		}

		if input == "help" {
			fmt.Println("Тут будет выводиться текст справки по нашему калькулятору")
		} else if res, err := Calculate(input); err == nil {
